...
\bigskip
\noindent\textbf{Badges requested.} We intend to apply for the SC26 badges \emph{Artifacts Available}, \emph{Artifacts Evaluated-Functional}, and \emph{Results Replicated}.
...
\item \textbf{Minimum reproduction target.} Reviewers with access to a single vendor fit inside SC26's 8-hour AE budget and can earn \emph{Results Replicated} credit on that vendor: NVIDIA GH200 or AMD MI300A alone reproduces all 21 rows of Table~IV for that column in $\sim$2--4~h; Intel PVC reproduces 18 rows (Kripke, llama.cpp, QuickSilver lack SYCL ports and are marked N/A). Per-vendor runs are independent; multi-vendor reproduction is parallelizable across separate GPU hosts.
