...
\begin{description}
\item[$A_1$] LEO analysis tool and evaluation infrastructure. Zenodo archive: \url{https://doi.org/10.5281/zenodo.19704349} (concept DOI --- always resolves to the latest release; the reviewed snapshot is tag \texttt{v0.1.6-sc26-ae} at \url{https://doi.org/10.5281/zenodo.19707117}). Pre-collected profiling data (separate deposit, $\sim$1~GB): \url{https://doi.org/10.5281/zenodo.19705577}.
\end{description}
...
RAJAPerf-tree kernels reuse the Table~IV entry point. Out-of-tree benchmarks live under \path{benchmarks/<name>/} and follow one of two patterns. (a)~Self-contained split (miniBUDE, XSBench, LULESH, and the rajaperf-h100 mirror of Table~IV): both \path{original/} and \path{optimized/} trees are vendored, so \texttt{diff -ruN original/ optimized/} shows the LEO change directly. (b)~Upstream-clone + patch (QuickSilver, llama.cpp, HipKittens, Kripke): the \path{optimized/} directory contains the modified files (and for llama.cpp / Kripke, explicit \path{*.patch} files); the per-benchmark \texttt{README.md} gives the upstream repo URL plus the specific commit SHA pinned at submission time, along with a one-line \texttt{git clone} + \texttt{git checkout} to obtain the baseline for diffing. Each benchmark has a \texttt{run\_compare.sh} (or per-vendor \texttt{run\_compare\_<vendor>.sh}) entry point.
