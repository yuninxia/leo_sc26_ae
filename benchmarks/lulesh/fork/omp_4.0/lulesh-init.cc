   ...
   m_refdens(Real_t(1.0))
{
   ...
   // Allocate gathered node data arrays for GPU-friendly contiguous access
   {
      Index_t numElem8 = numElem() * 8;
      m_gx  = static_cast<Real_t *>(malloc(sizeof(Real_t) * numElem8)) ;
      m_gy  = static_cast<Real_t *>(malloc(sizeof(Real_t) * numElem8)) ;
      m_gz  = static_cast<Real_t *>(malloc(sizeof(Real_t) * numElem8)) ;
      m_gxd = static_cast<Real_t *>(malloc(sizeof(Real_t) * numElem8)) ;
      m_gyd = static_cast<Real_t *>(malloc(sizeof(Real_t) * numElem8)) ;
      m_gzd = static_cast<Real_t *>(malloc(sizeof(Real_t) * numElem8)) ;
   }
...
Domain::CreateRegionIndexSets(Int_t nr, Int_t balance)
{
   ...
   // Sort each region's element list for contiguous memory access
   for (Int_t r=0 ; r<nr ; ++r) {
      std::sort(m_regElemlist[r], m_regElemlist[r] + regElemSize(r));
   }
}
