#!/bin/bash
# Compare original (upstream) vs optimized (fork) LULESH on AMD MI300A.
#
# Runs inside Docker container (leo-lulesh-amd) which already has the
# upstream LULESH pre-built. Only the optimized fork needs to be built.
#
# Usage:
#   ./run_compare_amd.sh                          # Build opt, compare (5 interleaved pairs)
#   ./run_compare_amd.sh --skip-build             # Reuse optimized build
#   ./run_compare_amd.sh --size 45 --iters 200    # Custom problem size
#   ./run_compare_amd.sh --pairs 20               # More measurement pairs
...
if [ "$SHOW_DIFF" = true ]; then
    ...
    echo ""
    # Compare fork against what the Docker image builds from (upstream 2.0.2)
    diff -ru "$SCRIPT_DIR/fork/omp_4.0/" "$SCRIPT_DIR/fork/omp_4.0/" 2>/dev/null || true
    echo "(Use 'git diff' inside fork/ to see actual changes vs upstream)"
    exit 0
fi
...
if [ ! -f /.dockerenv ]; then
    ...
    exec docker run --rm \
        ...
        -c "
            ...
            cp /opt/lulesh-compare/run_compare_amd.sh /opt/run_compare_amd.sh
...
if [ "$SKIP_BUILD" = false ]; then
    echo "============================================"
    echo " Building original + optimized LULESH (AMD)"
    echo "============================================"
    echo ""

    GPU_TARGET="${GPU_TARGET:-gfx942}"
    BUILD_ARGS='CXX=amdclang++ CXXFLAGS="-DUSE_MPI=0 -g -O2 -fopenmp -fopenmp-targets=amdgcn-amd-amdhsa -Xopenmp-target=amdgcn-amd-amdhsa -march='"${GPU_TARGET}"'" LDFLAGS="" OMPFLAGS=""'
...
echo "============================================"
echo " LULESH Original vs Optimized (AMD)"
echo " Args:      $LULESH_ARGS"
echo " Pairs:     $PAIRS (interleaved)"
