#!/bin/bash
# Compare original (upstream) vs optimized LULESH on NVIDIA H100.
#
# Runs inside Docker container (leo-lulesh-nvidia). Builds both original
# and optimized from source to include per-kernel timing instrumentation.
#
# Usage:
#   ./run_compare_nvidia.sh                          # Build both, compare (10 runs)
#   ./run_compare_nvidia.sh --skip-build             # Reuse builds
#   ./run_compare_nvidia.sh --size 45 --iters 200    # Custom problem size
#   ./run_compare_nvidia.sh --pairs 10               # Number of runs
...
DOCKER_IMAGE="leo-lulesh-nvidia"
...
if [ "$SHOW_DIFF" = true ]; then
    echo "Source differences (optimized vs upstream omp_4.0):"
    echo ""
...
if [ ! -f /.dockerenv ]; then
    ...
    exec docker run --rm \
        ...
        -c "
            ...
            cp /opt/lulesh-compare/run_compare_nvidia.sh /opt/run_compare_nvidia.sh
...
if [ "$SKIP_BUILD" = false ]; then
    echo "============================================"
    echo " Building original + optimized LULESH (NVIDIA)"
    echo "============================================"
    echo ""

    BUILD_ARGS='CXX=nvc++ CXXFLAGS="-DUSE_MPI=0 -fast -gopt -mp=gpu -gpu=cc90" LDFLAGS="" OMPFLAGS=""'
...
echo "============================================"
echo " LULESH Original vs Optimized (NVIDIA)"
...
run_batch() {
    ...
    for ((r=1; r<=n; r++)); do
        ...
        local t
        # Prefer per-kernel hourglass time; fall back to Elapsed time
