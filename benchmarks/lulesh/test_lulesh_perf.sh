#!/bin/bash
# Performance test: 10 warmups, 100 interleaved pairs
set -e
...
echo ""
echo "=== Warmup (10 runs each) ==="
for w in $(seq 1 10); do
    $ORIG $ARGS > /dev/null 2>&1
    $OPT $ARGS > /dev/null 2>&1
    printf "  warmup %2d/10 done\n" "$w"
done
echo "Done"

echo ""
echo "=== 100 interleaved pairs ==="
for r in $(seq 1 100); do
    ...
    printf "  Round %3d: orig=%ss  opt=%ss\n" "$r" "$ot" "$pt"
done
