...
SimulationData move_simulation_data_to_device( Inputs in, int mype, SimulationData SD )
{
	...
	// OPT: Energy-only array for cache-efficient binary search (8 bytes vs 48 bytes per element)
	sz = GSD.length_nuclide_grid * sizeof(double);
	gpuErrchk( cudaMalloc((void **) &GSD.nuclide_grid_energy, sz) );
	gpuErrchk( cudaMemcpy(GSD.nuclide_grid_energy, SD.nuclide_grid_energy, sz, cudaMemcpyHostToDevice) );
	total_sz += sz;
...
void release_device_memory(SimulationData GSD) {
	...
	cudaFree(GSD.nuclide_grid_energy);
...
SimulationData grid_init_do_not_profile( Inputs in, int mype )
{
	...
	// OPT: Extract energy-only array for cache-efficient binary search.
	// Binary search only compares energy (8 bytes) but NuclideGridPoint is 48 bytes.
	// Separate array gives 6x better cache line utilization during search.
	SD.nuclide_grid_energy = (double *) malloc( SD.length_nuclide_grid * sizeof(double) );
	assert(SD.nuclide_grid_energy != NULL);
	nbytes += SD.length_nuclide_grid * sizeof(double);
	for( int i = 0; i < SD.length_nuclide_grid; i++ )
		SD.nuclide_grid_energy[i] = SD.nuclide_grid[i].energy;
