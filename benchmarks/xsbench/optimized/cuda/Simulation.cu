...
__device__ void calculate_micro_xs(   double p_energy, int nuc, long n_isotopes,
                           ...
                           const double * __restrict__ nuclide_grid_energy,
	...
	if( grid_type == NUCLIDE )
	{
		...
		// check to ensure that we're not reading off the end of the nuclide's grid
	...
	else // Hash grid
	{
		...
		if( p_energy <= e_low )
			...
			lower = grid_search_energy( n_gridpoints, p_energy, nuclide_grid_energy, offset+u_low, offset+u_high);
...
// OPT: Binary search on energy-only array (8 bytes per element vs 48 bytes with NuclideGridPoint struct).
// This gives 6x better cache line utilization during the search loop since binary search
// only compares the energy field but NuclideGridPoint is 48 bytes (6 doubles).
__device__ long grid_search_energy( long n, double quarry, const double * __restrict__ energy_array, long low, long high)
{
	long lowerLimit = low;
	long upperLimit = high;
	long examinationPoint;
