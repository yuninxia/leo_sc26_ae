...
SimulationData move_simulation_data_to_device( Inputs in, int mype, SimulationData SD )
{
	...
	// OPT: Energy-only array for cache-efficient binary search on GPU
	sz = GSD.length_nuclide_grid * sizeof(double);
	gpuErrchk( hipMalloc((void **) &GSD.nuclide_grid_energy, sz) );
	gpuErrchk( hipMemcpy(GSD.nuclide_grid_energy, SD.nuclide_grid_energy, sz, hipMemcpyHostToDevice) );
	total_sz += sz;
...
SimulationData grid_init_do_not_profile( Inputs in, int mype )
{
	...
	for( long i = 0; i < SD.length_nuclide_grid; i++ )
