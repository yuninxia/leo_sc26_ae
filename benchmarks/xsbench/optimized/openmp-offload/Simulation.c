...
unsigned long long run_event_based_simulation(Inputs in, SimulationData SD, int mype)
{
	...
	map(to: SD.nuclide_grid_energy[:SD.length_nuclide_grid])\
  map(from: verification[:in.lookups])
	for( int i = 0; i < in.lookups; i++ )
	{
		...
		calculate_macro_xs(
				...
				SD.nuclide_grid_energy, // OPT: Energy-only array for cache-efficient binary search
...
void calculate_micro_xs(   double p_energy, int nuc, long n_isotopes,
	...
	if( grid_type == NUCLIDE )
	{
		...
		idx = grid_search_nuclide_energy( n_gridpoints, p_energy, &nuclide_grid_energy[nuc*n_gridpoints], 0, n_gridpoints-1);
	...
	else // Hash grid
	{
		...
		double e_low  = nuclide_grid_energy[nuc*n_gridpoints + u_low];
		double e_high = nuclide_grid_energy[nuc*n_gridpoints + u_high];
		int lower;
		if( p_energy <= e_low )
			lower = 0;
			...
			lower = grid_search_nuclide_energy( n_gridpoints, p_energy, &nuclide_grid_energy[nuc*n_gridpoints], u_low, u_high);
...
void calculate_macro_xs( double p_energy, int mat, long n_isotopes,
	...
	for( int j = 0; j < num_nucs[mat]; j++ )
	{
		...
		calculate_micro_xs( p_energy, p_nuc, n_isotopes,
		                    ...
		                    nuclide_grids, nuclide_grid_energy, idx, xs_vector, grid_type, hash_bins );
		for( int k = 0; k < 5; k++ )
...
// OPT: Binary search on energy-only array with hybrid linear scan for small ranges.
// Uses a separate double[] instead of NuclideGridPoint[] for 6x better cache utilization.
// For small ranges (<=8), linear scan avoids branch misprediction on wide-SIMD GPUs.
long grid_search_nuclide_energy( long n, double quarry, const double * A, long low, long high)
{
	long length = high - low;

	// OPT: Linear scan for small ranges — avoids branch misprediction
	if( length <= 8 )
	{
	...
	// Binary search on contiguous energy-only array
	long lowerLimit = low;
	long upperLimit = high;
	long examinationPoint;
