#!/bin/bash
# Compare original (upstream) vs optimized (fork) XSBench on NVIDIA H100.
#
# Runs inside Docker container (leo-xsbench-nvidia) which already has the
# upstream XSBench pre-built. Only the optimized fork needs to be built.
#
# Usage:
#   ./run_compare_nvidia.sh                          # Build opt, compare (3 runs)
#   ./run_compare_nvidia.sh --skip-build             # Reuse optimized build
#   ./run_compare_nvidia.sh --lookups 170000000      # More lookups for stable timing
#   ./run_compare_nvidia.sh --runs 5                 # Multiple runs
#   ./run_compare_nvidia.sh --device 0               # Use GPU device 0
#   ./run_compare_nvidia.sh --diff                   # Show source diff

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DOCKER_IMAGE="leo-xsbench-nvidia"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --vendor)      shift 2 ;;  # ignored, always nvidia
...
if [ "$SHOW_DIFF" = true ]; then
    echo "Source differences between original/ and optimized/ (CUDA):"
    echo ""
    diff -ru "$SCRIPT_DIR/original/cuda/" "$SCRIPT_DIR/optimized/cuda/" 2>/dev/null || true
    exit 0
fi
...
if [ ! -f /.dockerenv ]; then
    ...
    exec docker run --rm \
        ...
        -c "
            ...
            cp /opt/xsbench-compare/run_compare_nvidia.sh /opt/run_compare_nvidia.sh
            chmod +x /opt/run_compare_nvidia.sh
            cd /opt
            bash /opt/run_compare_nvidia.sh --vendor nvidia ${PASSTHROUGH_ARGS[*]}
        "
fi

# ==================== Inside Docker ====================
BIN_ORIGINAL="/opt/xsbench/bin/XSBench"
SRC_OPTIMIZED="/opt/xsbench-optimized/cuda"
...
if [ "$SKIP_BUILD" = false ]; then
    echo "============================================"
    echo " Building optimized XSBench (NVIDIA CUDA)"
    ...
    MAKE_CMD="make COMPILER=nvidia SM_VERSION=90 OPTIMIZE=yes CFLAGS=\"-Xcompiler -Wall -Xcompiler -O3 -arch=sm_90 -std=c++14 -lineinfo -g\""
...
echo "============================================"
echo " XSBench Original vs Optimized (NVIDIA)"
...
echo ""
echo "  Source diff:"
diff -rq "/opt/xsbench-original/cuda/" "/opt/xsbench-optimized/cuda/" 2>/dev/null | \
